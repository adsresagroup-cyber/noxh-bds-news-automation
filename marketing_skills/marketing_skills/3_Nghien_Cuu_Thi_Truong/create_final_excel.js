...
async function createFinalExcelReport() {
    ...
    sheet.columns = [
        ...
        { header: 'Tiêu đề bài báo', key: 'title', width: 45 },
        { header: 'Đường dẫn [URL]', key: 'url', width: 35 },
        { header: 'Tóm tắt nội dung', key: 'summary', width: 65 },
        { header: 'Góc nhìn phân tích', key: 'angle', width: 55 },
        { header: 'Kịch bản video', key: 'script', width: 90 }
    ];
...
Các bác nghĩ sao về xu hướng chuyển dịch dòng tiền này? Liệu phân khúc đất nền vùng ven Nghệ An còn có cơ hội tăng giá không? Hãy bình luận góc nhìn của các bác bên dưới và ấn theo dõi follow kênh nhé!`
    ...
    articles.forEach(item => {
        ...
        sheet.getRow(row.number).height = 160;
    });
    ...
    const outputPath = path.join('c:', 'Users', 'Admin', '.antigravity-ide', 'tin_tuc_bat_dong_san_nghe_an_final.xlsx');
    await workbook.xlsx.writeFile(outputPath);
    console.log('Final Excel report saved successfully to:', outputPath);
}

createFinalExcelReport().catch(console.error);
