                ...
                if ($null -ne $parsed -and -not [string]::IsNullOrEmpty($parsed.Voiceover)) {
                    ...
                    if ($rawWordCount -ge 230 -and $rawWordCount -le 290) {
                        ...
                        if ($rawWordCount -lt 230) {
                            $additionalHint = "[YÊU CẦU ĐIỀU CHỈNH ĐỘ DÀI]`nKịch bản vừa viết chỉ có $rawWordCount từ (dưới 230 từ). Vui lòng bổ sung thêm các phân tích chuyên sâu tác động thực tế, quy trình xét duyệt, kinh nghiệm nộp hồ sơ để tổng số từ NẰM CHÍNH XÁC TRONG KHOẢNG TỪ 230 ĐẾN 290 TỪ TIẾNG VIỆT."
                        } else {
