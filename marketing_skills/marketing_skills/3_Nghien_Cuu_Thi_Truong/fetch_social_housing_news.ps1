...
function Get-ConformingVoiceover {
    ...
    # Padding sentences if too short (specifically tailored to Social Housing topic)
    $paddingSentences = @(
        "Thị trường nhà ở xã hội thời điểm này đang thu hút sự quan tâm rất lớn từ người dân nhờ các chính sách hỗ trợ thiết thực.",
        "Quy trình đăng ký hồ sơ và điều kiện mua nhà ở xã hội luôn cần được tìm hiểu kỹ lưỡng để tránh các sai sót đáng tiếc.",
        "Các dự án nhà ở xã hội chất lượng cao đang mở ra cơ hội an cư lập nghiệp bền vững cho hàng ngàn gia đình lao động.",
        "Hãy theo dõi kỹ các thông tin quy hoạch và tiến độ triển khai để không bỏ lỡ đợt mở bán tiếp theo của dự án.",
        "Nếu các bác có bất kỳ thắc mắc nào về thủ tục làm hồ sơ mua nhà ở xã hội, hãy để lại ý kiến dưới phần bình luận nhé."
    )
...
- Để kiểm soát độ dài trong khoảng 230-290 từ, hãy cấu trúc phần Voiceover gồm đúng các phần:
  ...
  3. NỘI DUNG CHÍNH (BODY): 2-3 đoạn văn ngắn phân tích chi tiết thông tin, rủi ro hoặc chia sẻ kinh nghiệm hồ sơ (khoảng 180-220 từ).
...
"@
        ...
        # This item is a candidate but NOT selected to write a script (columns D, E, F, G left blank)
        $rows += [PSCustomObject]@{
            "STT" = $idx
            "Tiêu đề" = $item.Title
            "Đường dẫn" = $item.Link
